...
- 第 0、2 节说的“31 个全部在 ±3% 以内”，准确的说法是**已检查方向未超门**：冻结见证只搜最大特征值方向，没有查最软方向。
  双侧全谱只有中度 train13 和重度 train3。
- 完整准备成本（P + GP + GPU 编码逐例合计）中位约 75 s，最大约 267 s；第 2 节的“编码 0.9–18 s”不含 P 和 GP，不能当作端到端成本。
- 第 5 节的判断降级：随机扰动只说明无结构误差危险，不能证明结构化网络学不会。确定性路线先作为强基线，
  补齐双侧谱、完整成本和设计灵敏度后，再按瓶颈决定网络的位置。
