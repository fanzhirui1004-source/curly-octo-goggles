...
COVER_G = Path('/root/autodl-tmp/CLAUDE_TAKEOVER_20260923/COVER_G/runs')


def members(case, names):
    import os
    if os.environ.get('G_BODY_DIR'):  # deployment timing: the geometry stage recomputed in the same end-to-end run
        return {n: np.load(Path(os.environ['G_BODY_DIR']) / n) for n in names}
    if 'cover' in case:  # second batch: locally recomputed geometry stage (moments verified bitwise vs producer)
        verify = json.loads((COVER_G / (case + '_G_VERIFY.json')).read_text())
        if verify['status'] != 'MATCH':
            raise ValueError('COVER_G_NOT_VERIFIED')
        return {n: np.load(COVER_G / (case + '_G') / 'body' / n) for n in names}
