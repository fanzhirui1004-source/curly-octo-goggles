# A2_tail8 arm (replaces A1_sens3 in chain_diag2)
...
# A2_tail8: v2L1 + 8 Chebyshev sweeps of exact interior equilibrium after the network (fallback-A-type tail, experiment)
cfg A2_tail8 '{"model_args": {"smooth_k": 8, "smooth_alpha": 30.0}}'
env FUSED_HYPER=1 OPL_CONV_FP32=1 SENS_REASSOC=1 bash -c "cd /root/autodl-tmp/OPL/src_v2 && $PY -u train3.py $O/A2_tail8.json" > $O/A2_tail8.log 2>&1
echo "$(date +%T) TRAINED A2_tail8 rc=$?" >> $O/chain_arms.status
post A2_tail8
echo CHAIN_TAIL_DONE >> $O/chain_arms.status
