...
class MGNO(nn.Module):
    ...
    def _convs(self, Xc, t, gate, k0, level):
        ...
        dense = torch.zeros((m ** 3, B, F), device=dev)
        for j in range(self.cpl):
            dense.zero_(); dense[t['dense_idx']] = Xc
...
REGISTRY = {'diffusion': Diffusion, 'graphlift': GraphLiftModel, 'mgno': MGNO}
