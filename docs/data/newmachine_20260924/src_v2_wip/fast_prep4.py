"""Step 0 (operator learning): fast_prep3 plus the cut-band port.

Cut band = the 27 Q2 nodes of every active cell that carries a certified positive-area MACRO_CUT_FACE patch (the
cut plane restricted to material). Any coupling through the cut surface (loads now, a skin later) acts through the
trace of the Q2 field on the plane, which only involves these nodes. Saved as CUT_NODES.npy, CUT_CELLS.npy (indices
into CELL_INDICES) and CUT_PATCHES.json (plane patch metadata).
...
def main(workers, out, case):
    ...
    def _box_plane(p):                                  # FULL packets carry the plane x = 1 (a box face): no cut
        nrm = [Fraction(v) for v in p['normal']]
        return sum(v != 0 for v in nrm) == 1 and abs(Fraction(p['offset'])) in (0, 1)
    cut_patches = [p for p in topo['patches'] if p['tag'] == 'MACRO_CUT_FACE' and not _box_plane(p)]
    cut_cells = np.unique(np.asarray([lookup[tuple(map(int, p['parent']))] for p in cut_patches], dtype=np.int64))
    cut_nodes = np.unique(ids[cut_cells]) if len(cut_cells) else np.zeros(0, dtype=np.int64)
    row['cut_patch_keys'] = sorted(cut_patches[0].keys()) if cut_patches else []
    ...
    (d / 'CUT_PATCHES.json').write_text(json.dumps([{k: v for k, v in p.items()} for p in cut_patches], default=str))
    ...
    row.update(cells=int(len(cells)), nodes=int(len(nodes)), faces=int(len(faces)), box_nodes=int(len(box_nodes)),
               cut_cells=int(len(cut_cells)), cut_nodes=int(len(cut_nodes)),
               port_nodes=int(len(np.union1d(box_nodes, cut_nodes))))
