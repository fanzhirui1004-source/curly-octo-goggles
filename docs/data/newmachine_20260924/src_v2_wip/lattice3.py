...
def face_traction_weights(cell, axis, value, pts_per_elem=6):
    """Consistent nodal weights of a uniform unit traction on the box face x_axis = value (0 or 1, local), over material
    only (|phi| <= tau and inside the cut half-space): sum over quadrature points of w N_i."""
    from element_polyref import CUBE
    n = cell.n
    h = 1.0 / (n * pts_per_elem)
    ...
    tau = np.asarray(cell.taus, float) @ w8
    keep = np.abs(f) <= tau
    if cell.normal is not None and cell.is_cut.any():
        keep &= (P @ np.asarray(cell.normal, float)) <= cell.offset
    P = P[keep]
    c = np.minimum(np.floor(P * n).astype(np.int64), n - 1)
    lookup = {tuple(x): k for k, x in enumerate(cell.cells.tolist())}
    ok = np.asarray([tuple(x) in lookup for x in c.tolist()], dtype=bool)
    P, c = P[ok], c[ok]
    xi = P * n - c
    ...
    for oo in np.ndindex(3, 3, 3):
        ...
        ids = (g[:, 0] * M + g[:, 1]) * M + g[:, 2]
        loc = np.searchsorted(cell.nodes, ids)
        np.add.at(w, loc, h * h * L[:, 0, oo[0]] * L[:, 1, oo[1]] * L[:, 2, oo[2]])
    return w
...
class Lattice:
    def __init__(self, cells, config, n=32, log=print):
        ...
        F, labels, gate = [], [], []
        import os
        consistent = os.environ.get('LAT_LOADS') == 'consistent'
        for mi in range(len(cells)):
            gg, comp, priv = pos[mi]
            if consistent:                                               # traction-consistent face loads (reported)
                Cm = cells[mi]['cell']
                local_val = 0.0 if cells[mi]['offset'][lp] == 0 else 1.0     # the plane x_lp = 0 in lattice coordinates
                wv = face_traction_weights(Cm, lp, local_val)
                wp = wv[np.isin(Cm.nodes, Cm.port_node_ids)]
                self.face_load_outside = getattr(self, 'face_load_outside', []) + [float(1 - wp.sum() / max(wv.sum(), 1e-300))]
            for d in range(3):
                ...
                f = np.zeros(N)
                if consistent:
                    sel = np.arange(len(wp)) * 3 + d
                    np.add.at(f, idx[mi][sel], wp)
                else:
                    f[idx[mi][on]] = 1.0
                f /= f.sum()
                F.append(f); labels.append(f'{cells[mi]["label"]}_face{"xy"[lp]}0_{"xyz"[d]}' + ('_cons' if consistent else '')); gate.append(True)
        C0 = cells[0]['cell']
        if C0.is_cut.any():
