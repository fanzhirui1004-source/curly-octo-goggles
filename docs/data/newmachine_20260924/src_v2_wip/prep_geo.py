...
def one(body, data, case):
    ...
    try:
        ...
        if C.ni == 0:                                     # degenerate sliver: every node is a port, E = identity
            raise ValueError(f'NO_INTERIOR: all {len(C.nodes)} nodes are ports ({len(C.cells)} cells); nothing to learn')
