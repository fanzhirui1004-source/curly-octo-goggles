...
COARSEST_LIMIT = 8192
# ROUTE 1 (Claude, 2026-09-23): level-1 blocks may carry quadratic fields (up to 30 columns), so the block-width
# guard is raised from 12. Nothing else differs from hierarchy_strain_network_xcase.py.
BLOCK_LIMIT = 64
...
class BlockJacobi(nn.Module):
    ...
    def __init__(self, dimension: int, indices: Tensor, factors: Tensor, omega: float):
        super().__init__()
        if indices.ndim != 2 or indices.dtype != torch.long or indices.shape[1] > BLOCK_LIMIT:
            raise ValueError("block indices must be int64 [blocks,r<=BLOCK_LIMIT], -1 padded")
