#!/bin/bash
# Route 1: lattice acceptance gate on the GPU (cut cell 0013 at the free end of a cantilever, FULL parent neighbour).
T=/root/autodl-tmp/CLAUDE_TAKEOVER_20260923; S=$T/xcase_src_35; O=$T/R1_LAT2
...
mkdir -p $O
cd $S
while pgrep -f "[r]un_box.sh" > /dev/null; do sleep 15; done
...
C13=fresh_train_0013_cover01_r1; L1=$T/R1_L1
run 0013_poly $C13 polyref --lattice x,y --lattice-variants c16,c24,c32,c64,l16:$L1/0013_L16/LEARN.json,l24:$L1/0013_L24/LEARN.json
run 0013_body $C13 body --lattice x,y --lattice-variants c16,c24,c32,c64
echo "$(date -u +%FT%TZ) LAT2_DONE" >> $O/status
