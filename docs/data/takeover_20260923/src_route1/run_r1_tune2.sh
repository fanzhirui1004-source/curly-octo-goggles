...
mkdir -p $O
cd $S
while pgrep -f "[r]un_r1_tune.sh" > /dev/null; do sleep 20; done
...
run 0013_d22_lz_s08 $C13 2 2 --omega-mode lanczos --safety 0.8
run 0013_d11_lz $C13 1 1 --omega-mode lanczos
run 0020_d22_lz $C20 2 2 --omega-mode lanczos
run 0013_d22_sa $C13 2 2 --omega-mode lanczos --smooth-p1 1.3333333333
run 0013_d11_sa $C13 1 1 --omega-mode lanczos --smooth-p1 1.3333333333
run 0013_d22_sa2 $C13 2 2 --omega-mode lanczos --smooth-p1 1.3333333333 --smooth-p12 1.3333333333
run 0020_d22_sa $C20 2 2 --omega-mode lanczos --smooth-p1 1.3333333333
echo "$(date -u +%FT%TZ) E3B_DONE" >> $O/status
