"""Route 2, experiment D2: certify route 1's fixed-depth skeleton fields on one CutFEM+GP cell (no teacher in the bracket).

Same discrete setting and flux-free equilibration as D1 (dual_d1.py: partition of unity, rigid coarse Galerkin
correction, local Neumann problems with the ghost-face halo). The primal fields are not synthetic perturbations but
the skeleton's own fields: for each load, the exact boundary coordinates q* and the skeleton's interior extension
X_k(q*) at depth k, mapped back to background DOFs, w = P [q*; X_k(q*)] (built on the GPU by encode_r1.py --fields).
Then c - c_low = |w - u*|_K^2 = q*^T (S_hat_k - S) q* is the skeleton's action error along q*, and the gap
c_up - c_low certifies it from above without the teacher.
Usage: dual_d2.py prep <case> <out.npz>
       dual_d2.py certify <case> <H> <halo> <fields.npz> <out.json>
"""
...
if MODE == 'prep':
    outp = Path(sys.argv[3]); H = HALO = 0
else:
    H, HALO, FIELDS, outp = int(sys.argv[3]), int(sys.argv[4]), Path(sys.argv[5]), Path(sys.argv[6])
rec = dict(case=case, mode=MODE, H=H, halo=HALO)
...
if MODE == 'prep':
    np.savez(outp, F=Fs, Ustar=Ustar, compliance=comp, names=np.array(names), nodes=nodes)
    rec['seconds'] = time.perf_counter() - t0
    print(json.dumps(rec), flush=True)
    sys.exit(0)
fz = np.load(FIELDS, allow_pickle=False)
W, load_index, labels = fz['W'], fz['load_index'], [str(x) for x in fz['label']]
if W.shape[0] != nb:
    raise ValueError('FIELD_DIMENSION')
rec['fields'] = int(W.shape[1])
# control columns: the exact fields themselves (the bracket must close to rounding)
U0 = np.concatenate([W, Ustar], axis=1)
load_all = np.concatenate([load_index, np.arange(len(names))])
labels = labels + ['exact'] * len(names)
meta = [(names[int(li)], lab) for li, lab in zip(load_all, labels)]
FF = Fs[:, load_all]; Us = Ustar[:, load_all]; cs = comp[load_all]
...
# the primal field's own lower bound (before the coarse correction): this is the skeleton's certified-side number
c_low0 = 2 * np.einsum('ij,ij->j', FF, U0) - np.einsum('ij,ij->j', U0, K @ U0)
out = {}
for k, (name, lab) in enumerate(meta):
    out.setdefault(name, {})[lab] = dict(
        rel_energy_error_before=float(e0[k]), rel_energy_error_after_coarse=float(e1[k]),
        c_low_primal_rel=float((c_low0[k] - cs[k]) / cs[k]),
        c_low_rel=float((c_low[k] - cs[k]) / cs[k]), c_up_rel=float((c_up[k] - cs[k]) / cs[k]),
        certified_relative_gap=float((c_up[k] - c_low0[k]) / c_low0[k]),
...
for name in names:
    print(name, ' '.join('%s err %.2e low0 %+.2e up %+.2e gap %.2e eff %s' % (e, v['rel_energy_error_before'], v['c_low_primal_rel'], v['c_up_rel'],
          v['certified_relative_gap'], 'n/a' if v['efficiency'] is None else '%.2f' % v['efficiency']) for e, v in out[name].items()), flush=True)
