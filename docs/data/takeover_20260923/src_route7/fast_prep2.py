...
fast_prep2: the same with the vectorized local-support filter (fast_topology.compile_support_fast) in place of the
frozen compile_topology, and the output directory R7_05.
"""
...
import fast_trace, fast_gp, fast_topology
...
def main(case):
    ...
    topo, stats = fast_topology.compile_support_fast(c, workers)
    row['topology_seconds'] = time.perf_counter() - t0
    row['topology_stats'] = stats
