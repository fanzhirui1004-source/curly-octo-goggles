#!/bin/bash
# Route 7: superset stage (frozen runtime environment). Usage: run_fast_superset.sh <workers> <delta> <cases...>
...
O=$T/R7_07; mkdir -p $O
cd $W/src
OMP_NUM_THREADS=1 PYTHONDONTWRITEBYTECODE=1 $V -B $D/fast_superset.py $1 $O $2 ${@:3} >> $O/superset.log 2>&1
rc=$?
echo "$(date -u +%FT%TZ) superset rc $rc" >> $O/status
