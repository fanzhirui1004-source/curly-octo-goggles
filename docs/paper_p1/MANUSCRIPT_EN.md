...
A learned substructure must reproduce the interior deformation induced by its neighbours as well as the forces it transmits to them. We develop a geometry-conditioned neural displacement extension for cut thin-walled triply periodic minimal surface (TPMS) cells and couple it to an interior equilibrium correction. The extension is linear in retained displacement, reproduces rigid motion, and preserves all prescribed box-face and cut-band coordinates. Its energy in a stabilised three-dimensional cut finite element model defines a variational condensed stiffness. Classical Ritz orthogonality relates the stiffness excess to the squared interior extension error and its equilibrium residual. We connect this local relation to an assembled identity equating compliance discrepancy with reconstructed field-error energy, and to a participation-weighted bound on the influence of each cell. Fixed-weight correction and matched-load assembly comparisons examine correction accuracy and the separation between global and local response errors, with 80 unseen geometries providing a broader assessment of predictor accuracy. For a representative cut cell under consistent tractions, adding a coarse solve before eight smoothing steps reduces mean energy excess from 4.68% to 0.230%; eight additional pre-smoothing steps reduce it to 0.186%. In an assembly with a learned target and an exact neighbour, a separately trained predictor with eight smoothing steps gives compliance and target-cell sensitivity errors of 0.0014% and 3.89% under one neighbour-face load, while the target carries 0.13% of the exact assembled energy. Interior equilibrium thus provides a mechanical criterion for improving the learned operator, while the derivative weighting and local reference scale of thickness sensitivity explain why accurate global compliance can coexist with an appreciable local design error.
...
### 6.4. Reducing interior error with fixed network weights
...
### 6.7. Effect of restricting the retained representation
...
### 6.8. Workload dependence of computational cost
...
The fixed-weight experiments show why prediction and numerical correction can be useful together. Under the tested correction budgets, the learned starting field gives lower final energy error than a zero-interior start. The remaining error has components that relax slowly: in M1, the examined low Jacobi-scaled eigenvalues lie below the smoothing interval's lower endpoint. Adding a coarse update at the same smoothing-step count substantially reduces the energy excess, demonstrating the value of an additional interior subspace despite its extra solve cost. The spectral observations are consistent with this improvement and describe relaxation of the internal problem at fixed retained values.
...
Reuse and batching determine how those costs are amortised. Exact condensation reuses an interior factorisation; the learned model reuses geometry-dependent preparation and applies field operations for each query. Correction contributes to both the forward extension and its transpose. These differences make query count, batch size and geometry reuse central to an efficiency comparison. The separate deployment study with predictor D illustrates this workload dependence; the computational benefit of the coarse-corrected extension remains to be assessed at matched response accuracy.
...
Interior equilibrium provides a common mechanical criterion for learning and correcting the response of cut TPMS substructures. The proposed geometry-conditioned displacement extension preserves retained values and rigid motion, and its complete transpose defines a variational condensed stiffness. Classical energy orthogonality makes the stiffness excess quadratic in the interior extension error. At exact assembled equilibrium, the compliance discrepancy equals the complete reconstructed field-error energy, with local directional errors bounded in their global influence by energy participation.

The fixed-weight results demonstrate that an interior coarse update can reduce error that persists under polynomial smoothing. For M1 under consistent tractions, adding the coarse solve before the same eight smoothing steps reduces mean energy excess from 4.68% to 0.230%; eight additional pre-smoothing steps reduce it to 0.186%, from an uncorrected 13.5%. The learned initial field also gives lower final energy error than a zero-interior start under the tested correction budgets. These improvements preserve the retained assembly space and arise from correcting the predicted interior field.

Global compliance and local design accuracy remain distinct requirements. In the assembly study with the separately trained eight-step predictor, a target carrying 0.13% of the exact assembled energy has compliance and local sensitivity errors of 0.0014% and 3.89% under the same neighbour-face load. Participation weights the target's effect on global work, while stiffness derivatives and the local reference scale govern sensitivity accuracy. Equilibrium correction therefore controls an energy measure of the learned approximation; reliable local design evaluation additionally requires the recovered field and its dependence on the design parameters. Computational value must be assessed at these response requirements, including setup, repeated actions and the accuracy attained by the assembled solve.
