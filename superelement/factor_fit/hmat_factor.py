#!/usr/bin/env python3
"""Hierarchical low rank applied to the FACTOR R* instead of to A.

HMAT_ECONOMY4 reached +-3% by approximating A entry-wise, at 14.34e6 parameters (17.5% of dense).
But the gate is multiplicative on the factor, not additive on A (docs/ERROR_METRIC_20260916.md):
with Rhat = R* + E the whitened operator is X^T X for X = Rhat R*^-1, so

    eps_op ~ 2 ||X - I|| <= 2 ||E|| / sigma_min(R*)

and sigma_min(R*) = sqrt(lam_min(A)).  Approximating R* therefore buys a tolerance looser by
sqrt(cond(A)) = cond(R*) = 269 than approximating A, for the same gate.  Whether that converts
into fewer parameters depends on how compressible R* is, which is what this measures.

Rhat is NOT required to be triangular here: H = X^T X is positive semidefinite whatever Rhat is,
so the audit stays honest.  R* is not symmetric, so each ORDERED block pair is approximated
separately -- and blocks that are entirely zero (R* is upper triangular in the elimination order)
cost nothing, which the tolerance handles by itself.
...
TOLS = (1e-1, 3e-2, 1e-2, 3e-3, 1e-3, 3e-4)
...
def place(Ah, cover, s, t, I, J, M):
    """Write one ORDERED block.  R* is not symmetric, so no transpose is implied."""
...
def spectrum(Ah, Rinv, d):
    """Ah here is Rhat, so the whitened operator is X^T X with X = Rhat R*^-1."""
    X = Ah @ Rinv
    H = X.T @ X
    del X
    H = 0.5 * (H + H.T)
    ...
    out10 = int(((mu - 1).abs() > 0.10).sum())
    out3 = int(((mu - 1).abs() > 0.03).sum())
    return eps, div, neg, out3, out10, float(mu[0]), float(mu[-1])
...
def main():
    ...
    A = R.clone()          # the TARGET of the approximation is now the factor itself
    ...
    adm_u, near_u = block_partition(root)
    adm = [(s, t) for s, t in adm_u] + [(t, s) for s, t in adm_u if s.nid != t.nid]
    near = [(s, t) for s, t in near_u] + [(t, s) for s, t in near_u if s.nid != t.nid]
    idxs = {}
    for s, t in adm + near:
        for n in (s, t):
    ...
    for s, t in near:
        ...
        near_params += int((A[I][:, J] != 0).sum())      # R* is triangular: zero entries cost nothing
    ...
    eps, div, neg, o3, o10, mlo, mhi = spectrum(Ah, Rinv, d)
    ...
    tmin = min(TOLS); store = []; capped = 0; zero_blocks = 0
    for s, t in adm:
        I, J = idxs[s.nid], idxs[t.nid]
        M = A[I][:, J]
        if float(M.abs().max()) == 0.0:
            zero_blocks += 1; del M; continue
    ...
    print('svd pass %.0f s, %d/%d blocks at the rank cap %d, %d admissible blocks identically zero\n'
          % (time.time()-t0, capped, len(adm), RANK_CAP, zero_blocks), flush=True)

    print('%-8s %-11s %-9s %-11s %-11s %-7s %-7s %-10s %-10s %s'
          % ('tol', 'params', 'fracdense', 'eps_op', 'D/d', 'n>3%', 'n>10%', 'mu_min', 'mu_max', 'verdict'), flush=True)
    rows = []
    for tol in TOLS:
        ts = time.time()
        Ah = torch.zeros(d, d, dtype=torch.float64, device=V.DEV)
        ...
        print('%-8.0e %-11d %-9.4f %-11.4e %-11.4e %-7d %-7d %-10.3e %-10.3e %s'
              % (tol, params, params/dense_params, eps, div, o3, o10, mlo, mhi, v), flush=True)
        rows.append(dict(tol=tol, params=params, near_params=near_params, lr_params=lr_params,
                         ...
                         n_outside_3pct=o3, n_outside_10pct=o10, mu_min=mlo, mu_max=mhi))
    Path('/root/autodl-tmp/NEURAL_SCHUR/HMAT_FACTOR.json').write_text(json.dumps(
    ...
    print('\nwritten HMAT_FACTOR.json (%.0f s)' % (time.time()-t0), flush=True)
