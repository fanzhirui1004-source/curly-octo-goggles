"""Build packed-upper A^{1/2} labels in the exact layout of the Cholesky R_UPPER labels.
...
def main():
    ...
    report = dict(seat=args.seat, d=d, reference=str(ref),
                  scope='symmetric positive definite square root of A = R^T R; '
    ...
    report['square_residual'] = float((M @ M - A).norm() / A.norm())
    ...
    C = torch.linalg.solve_triangular(R, M.T, upper=True, left=False)   # C = M R^{-1}
    W = C.T @ C
    W = (W + W.T) * .5
    mu = torch.linalg.eigvalsh(W)
    report['label_eps_op'] = float((mu - 1).abs().max())
    del C, W, mu, A
    ...
    (ref / 'SQRT_RESULT.json').write_text(json.dumps(report, indent=1))
