#!/bin/bash
# First generalisation measurement: 226 presented, 40 never presented and never calibrated.
...
SEATS=$(grep '^SEATS=' "$G/plan.txt" | cut -d= -f2-)
TRAIN=$(grep '^TRAIN_SEATS=' "$G/plan.txt" | cut -d= -f2-)
EVAL=$(grep '^EVAL_SEATS=' "$G/plan.txt" | cut -d= -f2-)
SHA=$(find "$D/src_rebuild/stage_cutfem_m4" -name '*.py' | sort | xargs sha256sum | sha256sum | cut -c1-16)
echo "GEN source sha $SHA"
echo "GEN seats $(echo $SEATS | wc -w) presented $(echo $TRAIN | wc -w) eval $(echo $EVAL | wc -w)"
...
$PY -u -m stage_cutfem_m4.run \
  ...
  --head chol --seed 20260917 --source-sha "$SHA" \
  --output "$G/GEN_CHOL"
echo "GEN chol rc=$? $(date -Is)"
