...
OUT = Path('/root/autodl-tmp/CLAUDE_SENS_20260917'); OUT.mkdir(exist_ok=True)
...
def main():
    ...
    rows = {}
    for tol, Rhat, params in build_factor_approximations(label, R, DEV):
        ...
        row = dict(tol=tol, eps_op=eps_op, params=int(params), loads={})
        print('\n   tol %.0e   module eps_op %.4e   params %d' % (tol, eps_op, params), flush=True)
        for name in loads:
        ...
        (OUT / 'RESULT.json').write_text(json.dumps(dict(
            seat=328, d=int(d), q=int(q), assembled_dofs=int(N), shared_nodes=int(shared),
